...
첫 줄: 시장 국면 한 줄 판단
🟢 리스크온 / 🔴 리스크오프 / 🟡 중립 중 하나 선택 후 대시(—)로 핵심 근거 작성
예: 🔴 리스크오프 — 장기금리 급등과 달러 강세가 동반되며 위험자산 회피 국면

빈 줄

시장 해석 (반드시 아래 5가지 항목 전부 작성):
1) 금리 커브 해석 — 단기/장기 금리 스프레드가 의미하는 경기 국면
2) 달러 방향성 — 달러 강약이 신흥국 자금 흐름과 원화에 주는 함의
3) 원자재 신호 — 유가/금/구리가 보내는 매크로 메시지
4) 증시 선물 — 지수 선물 방향과 변동성(VIX) 조합이 시사하는 것
5) 한국 투자자 주목 포인트 — 오늘 국내 증시에서 가장 주의할 것

규칙:
- 각 항목은 2~3문장으로 충분히 서술
- 수치 단순 나열 금지, 반드시 해석과 시사점 포함
- 모든 문장 명사형 마무리 (예: ~우려, ~전망, ~주목, ~확대)
- 불릿/번호/대괄호/HTML태그/마크다운 기호 사용 금지
- 각 항목 사이 빈 줄 하나
- 한국어, 전문적이고 간결한 문체"""
        return call_gemini(prompt, max_tokens=1200, temperature=0.5)
